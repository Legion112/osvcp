...
#define THRESHOLD 1000000  // Threshold for sloppy counter

typedef struct {
    int global_count;           // Global counter
    pthread_mutex_t global_lock; // Lock for global counter
    int *local_counts;           // Per-thread local counters
...
void sloppy_counter_init(sloppy_counter_t *counter, int num_threads) {
    ...
    counter->local_counts = (int *)malloc(num_threads * sizeof(int));
    counter->local_locks = (pthread_mutex_t *)malloc(num_threads * sizeof(pthread_mutex_t));
    counter->num_threads = num_threads;

    for (int i = 0; i < num_threads; i++) {
        counter->local_counts[i] = 0;
        pthread_mutex_init(&counter->local_locks[i], NULL);
    }
}

// Update the counter (increment by 1)
void sloppy_counter_update(sloppy_counter_t *counter, int thread_id) {
    pthread_mutex_lock(&counter->local_locks[thread_id]);
    counter->local_counts[thread_id]++;
    if (counter->local_counts[thread_id] >= THRESHOLD) {
        // Transfer local to global
        pthread_mutex_lock(&counter->global_lock);
        counter->global_count += counter->local_counts[thread_id];
        pthread_mutex_unlock(&counter->global_lock);
        counter->local_counts[thread_id] = 0;
    }
    pthread_mutex_unlock(&counter->local_locks[thread_id]);
}
...
void *thread_func(void *arg) {
    ...
    for (int i = 0; i < args->increments; i++) {
        sloppy_counter_update(args->counter, args->thread_id);
    }
    return NULL;
}
